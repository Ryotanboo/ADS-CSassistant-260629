...
import type { ChatMessage, Consultation, Customer } from "@/lib/cs-schema";
...
- 可能な限り「ここまでの整理」と「次にやること」で締める

## 次アクションへの着地
- 相談が整理できたら、「次にやること」を1〜3個の短い候補として提示する
- 各候補は実行できる粒度にする（例: 「次回MTGで決裁者の判断基準を確認する」）
- まだ情報が足りない場合は、次アクションではなく「未確認事項」として分ける
- Pane 4 のネクストアクションに追加される前提で、短く分かりやすい表現にする
- 次アクション候補を出すときは、本文の最後に「次にやること」として箇条書きにする
- 次アクション候補を出した後は、原則として追加の質問で会話を引き延ばさない
